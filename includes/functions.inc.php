...
function get_month_name($month, $secondForm) {
    $months = array(
        array('Січень', 'Січня'),
        array('Лютий', 'Лютого'),
        array('Березень', 'Березня'),
        array('Квітень', 'Квітня'),
        array('Травень', 'Травня'),
        array('Червень', 'Червня'),
        array('Липень', 'Липня'),
        array('Серпень', 'Серпня'),
        array('Вересень', 'Вересня'),
        array('Жовтень', 'Жовтня'),
        array('Листопад', 'Листопада'),
        array('Грудень', 'Грудня')
    );
    return $months[$month][$secondForm ? 1 : 0];
}

function get_month_name_pl($month) {
    $months = array(
        'Styczeń',
        'Luty',
        'Marzec',
...
function weekTitle($weekNum) {
    $weekDisplayed = new DateTime();
    $weekDisplayed->setISODate(2017, $weekNum);
    return $weekDisplayed->format('Y') . ' ' . get_month_name($weekDisplayed->format('n') - 1, false) . ' - Тиждень ' . $weekDisplayed->format('W');
}